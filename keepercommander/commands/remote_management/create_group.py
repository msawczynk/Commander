...
from keepercommander.discovery_common.rm_types import RmAwsGroupAddMeta, RmAzureGroupAddMeta, RmGroup
...
class GatewayActionRmCreateGroupCommandInputs:
...
class GatewayActionRmCreateGroupCommand(GatewayAction):

    def __init__(self, inputs: GatewayActionRmCreateGroupCommandInputs, conversation_id=None):
        super().__init__('rm-create-group', inputs=inputs, conversation_id=conversation_id, is_scheduled=True)

    def toJSON(self):
        return json.dumps(self, default=lambda o: o.__dict__, sort_keys=True, indent=4)


class RmCreateGroupCommand(PAMGatewayActionDiscoverCommandBase):
    parser = argparse.ArgumentParser(prog='pam-rm-group-create')
    ...
    def get_meta_role_class(params: KeeperParams,
        ...
        meta_provider_map = {
            "pamAwsConfiguration": RmAwsGroupAddMeta,
            "pamAzureConfiguration": RmAzureGroupAddMeta,
        }
        ...
        if record.record_type == "pamDatabase":
            raise Exception("Databases do not support groups.")
    ...
    def get_parser(self):
        return RmCreateGroupCommand.parser
    ...
    def execute(self, params: KeeperParams, **kwargs):
        ...
        action_inputs = GatewayActionRmCreateGroupCommandInputs(
        ...
        router_response = router_send_action_to_gateway(
            params=params,
            gateway_action=GatewayActionRmCreateGroupCommand(
        ...
        if data is None:
            ...
            print(f"{bcolors.FAIL}Could not create group: {error}{bcolors.ENDC}")
            return

        group_json = gateway_context.decrypt(data.get("data"))
        group = RmGroup.model_validate(group_json)

        print(f"{bcolors.OKGREEN}Group created successfully{bcolors.ENDC}")
        print("")
        print(f"{bcolors.BOLD}Id{bcolors.ENDC}: {group.id}")
        print(f"{bcolors.BOLD}Name{bcolors.ENDC}: {group.name}")
        print("")
